...
export function DemoCTA() {
  return (
    <div className="overflow-hidden rounded-2xl border border-[var(--color-accent)]/20 bg-gradient-to-br from-[var(--color-accent-soft)] via-white to-[var(--color-brand-soft)] p-4 shadow-[var(--shadow-card)] sm:p-6">
        ...
        <div className="grid gap-3 text-sm">
          {[
            ...
            <Link
              key={item.href}
              href={item.href}
              className="flex items-center gap-2.5 rounded-xl border border-white/60 bg-white/75 px-3 py-2.5 font-semibold text-slate-700 backdrop-blur-sm transition hover:border-[var(--color-accent)]/40 hover:bg-white hover:text-[var(--color-accent)] sm:px-4"
            >
